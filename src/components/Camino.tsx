...
export function Camino() {
  ...
  const { CAMINO } = t
  return (
    <section id="camino" className="border-t border-cookie/15 py-24 sm:py-36">
      <div className="marco">
        <Reveal>
            ...
            <Reveal key={h.titulo + i} delay={i * 90} className="relative border-l border-cookie/20">
              <li className="grid grid-cols-1 gap-x-8 gap-y-2 pb-16 pl-8 sm:grid-cols-[9rem_1fr] sm:pl-10">
                ...
                <div className="sm:pt-0">
                  <h3 className="flex items-center gap-2.5 font-hn text-lg leading-7 text-cookie sm:text-xl">
                    {h.icono && <Logo src={h.icono} size="md" />}
                    {h.titulo}
                  </h3>
