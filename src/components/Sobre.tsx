...
export function Sobre() {
  ...
  const { SOBRE } = t
  return (
    <section id="sobre" className="border-t border-cookie/15 py-24 sm:py-36">
      <div className="marco">
        <Reveal>
          ...
          <div className="max-w-2xl space-y-5">
            {SOBRE.parrafos.map((p, i) => (
              <Reveal key={p} delay={200 + i * 80}>
                <p className="font-hn text-base leading-relaxed text-cookie/70 sm:text-lg">
                  <Texto>{p}</Texto>
                </p>
              </Reveal>
            ))}
            ...
            <dl className="space-y-5 sm:min-w-[15rem]">
              {SOBRE.datos.map((d) => (
                <div key={d.clave} className="border-t border-cookie/15 pt-3">
                  ...
                  <dd className="mt-1 flex items-center gap-2 font-hn text-sm text-cookie">
                    {d.icono && <Logo src={d.icono} size="md" />}
                    {d.valor}
                  </dd>
                </div>
              ))}
